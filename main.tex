...
prognosis. Its morphology is heterogeneous, but prototypically consists of an inner, largely necrotic core surrounded by an outer, contrast-enhancing rim, and often extensive tumor-associated edema beyond. This structure is usually demonstrated by magnetic resonance (MR) imaging.  To help relate the three highly idealized components of GBMs (i.e. necrotic core, enhancing rim, and maximum edema extent) and to the underlying growth ``laws,'' a mathematical
...
the wave profile, which can be compared with MR imaging data. Several simplified
...
rim is usually surrounded by a large expanse of edema that is best visualized on T2-weighted MRI, and may correspond to diffusely invasive, highly motile
...
of (\ref{eq:main3d}) stay nonnegative and is bounded by $p+q\le1$
for all $t$, supplemented with suitable initial condition.
...
(\ref{eq:l1}) over (\ref{eq:l2}) same as previously and get (\ref{eq:f(rho/k)}). It can then be solved for the ratio $\hat{\rho}=\rho/k$. Substituting this back to either $\ell_1=L1$ or $\ell_2=L2$ gives the raito $\rho/D$. Lastly, expressing $D$ and $k$ in terms of $\rho$, (\ref{eq:age est}) can be solved for $\rho$, and $D$ and $k$ follows.

\begin{figure}
\begin{center}
\includegraphics[scale=1]{plots/radii_process.eps}\caption{\label{fig:radii_process} The top half of the figure shows an example patient MRI registered to the standard brain domain, with the three tumor segments, necrotic core, enhancing rim, and tumor-associated edema highlighted on a single 2-D slice.  The full 3-D segmentation, and the equivalent tumor sphere with associated radii, $R_0$, $R_1$, and $R_2$, is shown on the figure lower half.}
\end{center}
\end{figure}
...
studied in a more general and theoretical context \cite{Sherratt2000}, and in \cite{Sherratt2001}, the authors justified the adoption of this
...
by a grant from Arizona Biomedical Research Center. \\
