...
an ansatz $p=Ae^{-r\xi}$ and $q=Be^{-r\xi}$ into (\ref{eq:in wave cord})
...
suits our purposes.  Therefore, we let $g(w)=B(w;\alpha_g,\beta_g)$ and $\delta(w)=1-B(w;\alpha_{\delta},\beta_{\delta})$, where $B(w; \alpha, \beta)$ is the CDF of Beta distribution with shape parameters $\alpha$ and $\beta$. By tweaking $\alpha$ and $\beta$, we can get linear, sigmoid and concave
...
The main purpose of this subsection is to rigorously establish the existence of  traveling wave solutions in system (\ref{eq:pp system}). To this end, we first show that the solutions of system (\ref{eq:pp system}) 
...
The trajectory in Figure \ref{fig:pp} corresponds to a traveling wave profile in $z-$coordinate as shown on the left pane of Figure \ref{fig:Match wid}. Because of our choice of nondimensionlization ($x=-\frac{2 \sqrt{D\rho}}{k} z$), by definition we have the rim width in dimensional form  
\begin{eqnarray}
\ell_1= \frac{2 \sqrt{D\rho}}{k} (z_1^+ - z_1^-) = \frac{2 \sqrt{D\rho}}{k} \int_{z_1^-}^{z_1^+} dz\
\end{eqnarray}
where $p(z^{\pm}_1)=a_1 p_{\max}$. Note that we have taken $p$ as a function of $z$ (see left pane of Figure \ref{fig:Match wid}). We can also take $w$ as a function of $z$ to make a change of variable to the above integral, resulting in (\ref{eq:l1}). Similar argument applies to (\ref{eq:l2})
 
\subsection*{Derivation of $R_1^*$ and $R_2^*$}

\begin{figure}[h!]
\begin{center}
\includegraphics[scale=0.90]{plots/wave.eps}\caption{\label{fig:wave}The transition of wave profile based on the two-stage tumor growth model. The dotted (purple) curve represents stable wave profile which generated by system (\ref{eq:pp system}). The solid curves are generated by (\ref{eq:exp system}) and represent tumor's exponential growth. The stable wave profile is formed after the exponential growth curve reaches $p_{max}$ (red solid curve).}
\end{center}
\end{figure}

Tumor growth can be seperated into two stages. First, the tumor cells will have exponential growth till the cell density is high enough ($p_{max}$) to form stable wave profile. Then the growth of tumor cells is described as (\ref{eq:pp system}). $R_1$ and $R_2$ are observed from clinical MRI data, which describe the growth of tumor cells from $t=0$. $R_1^*$ and $R_2^*$ are used to describe the radius before the stable wave profile has formed (see in Figure \ref{fig:wave}). 

Assuming when $0<t<t^*$, quiescence is negligible and the proliferating cell density grows exponentially, the governing equation of tumor cell density is 
\begin{eqnarray}\label{eq:exp system}
\pardiff pt =  D\frac{1}{r^2} \pardiff{}{r} (r^2 \pardiff pr) + \rho p,
\end{eqnarray}
where spherical symmetry is used since spherical effect is important when tumor radius is small. This linear equation has a Green's function 
\begin{eqnarray}
p(r,t)=\frac{1}{(4 \pi D t)^{3/2}} e^{\rho t - \frac{r^2}{4Dt}} 
\end{eqnarray}
(see, e.g., \cite{KotBook} Page 314 and \cite{BrittonBook} Page 285). Suppose that the tumor starts at $t=0$ as a point source with density $p_0$. It follows that at $t=t^*$, the position of tumor front at density $p=a_i$  is as (\ref{eq:Rstar}). 
